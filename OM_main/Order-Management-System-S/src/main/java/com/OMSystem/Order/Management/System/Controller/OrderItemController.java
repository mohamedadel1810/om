...
public class OrderItemController {
    ...
    private OrderItemService orderItemService ;
         ...
         @GetMapping
    public ResponseEntity<ApiResponse<List<OrderItemResponseDto>>>getAll(){
             return ResponseEntity.ok(orderItemService.findAll());
         }
         @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<OrderItemResponseDto>>getById(@PathVariable int id){
             return ResponseEntity.ok(orderItemService.findById(id));
         }
    @PostMapping
    public ResponseEntity<ApiResponse<OrderItemResponseDto>> addProduct(@RequestBody OrderItemRequestDto item){

        return ResponseEntity.ok(orderItemService.save(item));
    }
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<OrderItemResponseDto>>updateProduct (@PathVariable int id ,@RequestBody OrderItemRequestDto item){
//        item.setId(id);
